...
public class ImageLoader {
    ...
    private static BufferInfo fromBufferedImage(BufferedImage image) {
        int width = image.getWidth();
        int height = image.getHeight();
        BufferedImage bgrimg = new BufferedImage(width, height, BufferedImage.TYPE_3BYTE_BGR);
        bgrimg.setRGB(0, 0, width, height, image.getRGB(0, 0, width, height, null, 0, width), 0, width);
    ...
    public static BufferInfo getBGRFromClasspathFile(String file) throws IOException {
        try (InputStream stream = ImageLoader.class.getClassLoader().getResourceAsStream(file)) {
